...
namespace cbhk_environment.Generators.SpawnerGenerator
{
    public class spawner_datacontext: ObservableObject
    {
        ...
        //刷怪笼标签页集合
        public ObservableCollection<RichTabItems> SpawnerPages { get; set; } = new() { new RichTabItems() { Header = "刷怪笼", Style = Application.Current.Resources["RichTabItemStyle"] as Style,IsContentSaved = true } };
        ...
        {
            await GenerateAllSpawner();
        }

        private async Task GenerateAllSpawner()
        {
            Result = new();
