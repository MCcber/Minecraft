...
namespace cbhk_environment.Generators.DataPackGenerator.Components.EditPage
{
    ...
    public class AddFileFormDataContext:ObservableObject
    {
        ...
        {
            if (NewItemStyleSource == null)
            {
                NewItemStyleSource = Window.GetWindow(sender as TreeView).FindResource("NewItemSourceCollection") as CollectionViewSource;
        ...
        private void NewItemStyleSource_Filter(object sender, FilterEventArgs e)
        {
            ...
            if(CurrentFileType != null)
            {
                if(!CurrentFileType.Uid.Contains('\\') && newItemStyle.NameSpace.Contains('\\'))
                    ...
                    e.Accepted = (CurrentFileType.Uid == newItemStyle.NameSpace || CurrentFileType.Uid.Length == 0) && (SearchText.Length == 0 || newItemStyle.FunctionName.Contains(SearchText));
            }
        }
